...
from utils.tts_tokenizers import BaseTokenizer, EnglishPhonemesTokenizer

from nemo.collections.tts.helpers.helpers import load_speaker_emb
...
class TTSDataset(Dataset):
    def __init__(
        ...
        sample_rate: int,
        text_tokenizer: Union[BaseTokenizer, Callable[[str], List[int]]] = None,
        ...
        **kwargs,
        ...
        self.speaker_emb_path = speaker_emb_path
    ...
    def add_pitch(self, **kwargs):
        ...
        self.use_log_pitch = kwargs.pop("use_log_pitch", False)
        self.unvoiced_value = kwargs.pop("unvoiced_value", None)
        if not self.unvoiced_value and not self.use_log_pitch:
            # need to compute unvoiced value
            if self.pitch_norm and self.pitch_mean is not None and self.pitch_std is not None:
                self.unvoiced_value = (0.0 - self.pitch_mean)/self.pitch_std
            logging.info(f"Unvoiced value used for this dataset is {self.unvoiced_value}")
        elif self.use_log_pitch:
            self.unvoiced_value = 0.0
            logging.info(f"Using log pitch for pitch as `use_log_pitch=True`...")
            logging.info(f"Unvoiced value used for this dataset is {self.unvoiced_value}")
            
        self.pyyapt_config = {'f0_min': self.pitch_fmin, 'f0_max': self.pitch_fmax, 
                'frame_length': int(self.win_length*1000/self.sample_rate), 
                'fft_length': self.n_fft,
                'frame_space': int(self.hop_length*1000/self.sample_rate),
                "tda_frame_length": int(self.win_length*1000/self.sample_rate),
                }
    ...
    def add_speaker_emb(self, **kwargs):
        if self.speaker_emb_path is not None:
            self.speaker_emb_dict = load_speaker_emb(self.speaker_emb_path)
        else:
            raise ValueError(f"Speaker embedding dictionary not found, provide a 'speaker_emb_path' in config.")
    ...
    def __getitem__(self, index):
        ...
        if AlignPriorMatrix in self.sup_data_types_set:
            try:
                ...
                logging.info(f"sample could not be processed {sample['audio_filepath']}")
                mel_len = 50 # dummy number, all samples here are eliminated from training set.
            if self.use_beta_binomial_interpolator:
        ...
        if SpeakerEmb in self.sup_data_types_set:
            audio_path = sample["audio_filepath"].split("/")[-1]
            audio_path = audio_path.split(".")[0]
            if (
                self.speaker_emb_dict is not None
                and audio_path in self.speaker_emb_dict
            ):
                speaker_emb = self.speaker_emb_dict.get(audio_path)
                audio_path = None
            else:
                raise ValueError(f"Speaker embedding for {audio_path} not found in dict")
    ...
    def general_collate_fn(self, batch):
        ...
        audios, tokens, log_mels, durations_list, pitches, energies, speaker_ids, speaker_embeddings, voiced_masks, p_voiceds = (
            [],
            [],
            [],
            [],
        ...
        data_dict = {
            ...
            "speaker_emb":  torch.stack(speaker_embeddings) if SpeakerEmb in self.sup_data_types_set else None,
...
def load_speaker_emb(path):
    logging.info(f"Loading external speaker embeddings from {path}")
    if path.split(".")[-1] == "pkl":
        with open(path, "rb") as read_file:
            speaker_embeddings_dict = pickle.load(read_file)
